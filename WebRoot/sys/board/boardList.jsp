				...
				<c:choose>
					<c:when test="${not empty requestScope.list }">
						<c:forEach items="${requestScope.list }" var="table"
							varStatus="vs">
							<tr class="TalbeDetail" align="center">
								<td >${vs.count }</td>
								...
								<td><c:choose>
										<c:when test="${table.tableStatus==1 }">
											<a
												href="${pageContext.request.contextPath }/table?method=update&id=${table.id}"
									...
									href="${pageContext.request.contextPath }/table?method=delete&id=${table.id}"
